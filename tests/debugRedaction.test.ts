...
test("redaction logs: ws_key masque dans les URLs", async () => {
  ensureCoreEnv();
  const { sanitizeUrlForLogs } = await import("../app/utils/debug");
  const sanitized = sanitizeUrlForLogs(
    "https://btob.wearmoi.com/api/orders?ws_key=secret123&display=[id]",
  );
  assert.equal(sanitized.includes("secret123"), false);
  assert.match(sanitized, /ws_key=\*\*\*/);
});
