...
async function loadUpdateEtaActionRoute() {
  ensurePurchaseOrdersActionEnv();
  return import("../app/routes/actions.purchase-orders.$purchaseOrderGid.update-eta");
}

test("API modifier-eta renvoie 400 si id manquant", async () => {
  const route = await loadUpdateEtaActionRoute();
  const response = await route.action({
    request: new Request("https://app.test/actions/reassorts-magasin//modifier-eta", {
      method: "POST",
    }),
    params: {},
  } as never);
...
test("API modifier-eta renvoie 400 si id invalide", async () => {
  const route = await loadUpdateEtaActionRoute();
  const response = await route.action({
    request: new Request("https://app.test/actions/reassorts-magasin/invalid/modifier-eta", {
      method: "POST",
    }),
